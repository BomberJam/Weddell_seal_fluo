...
# prendre la dernière valeur non-NA de `light_raw` 
last_depths <- filtered_data %>%
  group_by(profile_id) %>%  
  filter(!is.na(light_raw)) %>%
  slice_tail(n = 1) %>%
  ungroup()

# profondeur maximale atteinte pour chaque profil
ggplot(last_depths, aes(x = date, y = depth)) + 
...
ggplot(data, aes(x = date, y = MLD, color = seal_id)) +
  geom_point(shape = 4) +
...
# Graphique avec facettes
ggplot(data, aes(x = date, y = QD, color = seal_id)) +
  geom_point(shape = 4) +
  geom_smooth(method = "loess", formula = "y ~ x", linewidth = 0.25, se = FALSE) +
  scale_y_reverse() +
  scale_x_date(date_labels = "%b %Y") +
