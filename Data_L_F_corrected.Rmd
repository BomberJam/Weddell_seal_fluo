...
DEPLOY_FOLDER <- "wd20" # Lignes à modifier pour changer de dépoliement...
SEAL_NAME <- paste0(DEPLOY_FOLDER,"-546-19") # ...ou d'individu
...
large_topo_spat <- crop(large_topo_spat, extent(136, 146, -67.3, -65.4))
```
...
raster_end <- lead(raster_time) - 1  # fin = début du suivant - 1 jour

raster_end[length(raster_end)] <- raster_start[length(raster_end)] + months(1) - 1
raster_range <- which(raster_start <= date_max & raster_end >= date_min)
raster_time[raster_range]
...
x_sub_p=x_sub
x_sub_p[x_sub_p == 0 | x_sub_p == 2 | x_sub_p == 3 | x_sub_p == 5| x_sub_p == 6] <- NA
...
x_sub2_long <- pivot_longer(x_sub2_df, cols = starts_with(AMSR_FOLDER_YEAR), names_to = "date_simple", values_to = "value")%>%
  mutate(
...
target_year_months <- c("202302", "202303", "202304", "202305", "202306", "202307", "202308", "202309") #format(raster_time[raster_range], "%Y%m")
...
list_SIC <- lapply(target_year_months, function(m) {
  ...
  rasters <- lapply(selected_files, function(f) {
    ...
    boxext <- extent(projectExtent(raster(extent(c(130,144,-67,-65)), crs = wgs), projection(r)))
...
list_SIC[[1]]$mean[is.na(list_SIC[[1]]$mean[])] <- 0
list_SIC[[1]]$median[is.na(list_SIC[[1]]$median[])] <- 0
list_SIC[[1]]$sd[is.na(list_SIC[[1]]$sd[])] <- 0
...
raster_to_df <- function(r, month_label, date, stat_name) {
  ...
  df <- df %>% filter(!is.na(.data[[stat_name]]), !is.nan(.data[[stat_name]]))
...
list_dfs <- lapply(list_SIC, function(entry) {
  ...
  if (nrow(mean_df) == 0 || nrow(median_df) == 0 || nrow(sd_df) == 0) return(NULL)

  # Trouver les coordonnées communes à tous les 3
  common_coords <- reduce(list(mean_df, median_df, sd_df), ~ inner_join(.x, .y, by = c("x", "y"))) %>%
    dplyr::select(x, y)

  # Filtrer chaque df pour ne garder que les coordonnées communes
  mean_df_filt <- semi_join(mean_df, common_coords, by = c("x", "y"))
  median_df_filt <- semi_join(median_df, common_coords, by = c("x", "y"))
  sd_df_filt <- semi_join(sd_df, common_coords, by = c("x", "y"))

  # Fusionner
  df_month <- mean_df_filt %>%
    left_join(median_df_filt, by = c("x", "y", "month_label", "date")) %>%
    left_join(sd_df_filt, by = c("x", "y", "month_label", "date"))
