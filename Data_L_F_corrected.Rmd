...
ggplot(df_max, aes(x = profile, y = value)) +
  ...
  geom_smooth(aes(y = value), method = "loess", formula = "y ~ x", color = "darkgreen", linewidth = 0.25, se = FALSE) +
  scale_y_reverse() +
  labs(x = "Profile", y = "Depth (m)", title = paste0("Max Chla depth - ", SEAL_NAME)) +
  theme_minimal()
...
```{r SaveAndClassifyChlaProfiles, results = 'hide'}
# Dossiers de sortie
...
df_plot <- x_sub2_long_crop %>%
  dplyr::filter(grepl(mois_cible, date_simple))
...
df_SIC <- do.call(rbind, lapply(list_SIC, function(entry) {
  s <- c(entry$mean, entry$median, entry$sd)
  names(s) <- c("mean", "median", "sd")
...
```{r}
idMap <- 1
plot(list_SIC[[idMap]]$mean, main = paste0("SIC Mean (", list_SIC[[idMap]]$month_label,")"))
plot(list_SIC[[idMap]]$median, main = paste0("SIC Median (", list_SIC[[idMap]]$month_label,")"))
plot(list_SIC[[idMap]]$sd, main = paste0("SIC SD (", list_SIC[[idMap]]$month_label,")"))
...
# Récupérer la valeur "fast_ice_idx" correspondante
df_final$fast_ice_idx <- x_sub2_long_crop$value[nn$nn.index[, 1]]

# SIC
# Extraire coords pour df_final (profil)
coords_profiles <- df_final[, c("lon", "lat")]
coords_SIC <- df_SIC[, c("x", "y")]

# nearest neighbor spatial (1 plus proche)
nn_spatial <- get.knnx(coords_SIC, coords_profiles, k = 1)

# Ajouter les colonnes SIC associées à df_final
...
df_at_4m <- subset(df_deployment_data, depth == 4)
light_at_4m <- df_at_4m[!is.na(df_at_4m$light_corrected), ]

light_at_4m$month_label <- format(light_at_4m$date, "%Y - %B")
...
ggplot() +
 ...
 # new_scale_fill() +
  ...
  new_scale_color() +
  geom_point(data = light_at_4m, aes(x = lon, y = lat, color = light_corrected), size = 1) +
...
df_at_4m <- subset(df_deployment_data, depth == 4)
chla_at_4m <- df_at_4m[!is.na(df_at_4m$chla_corrected), ]

chla_at_4m$month_label <- factor(format(chla_at_4m$date, "%Y - %B"),
                                 levels = c("2019 - February", "2019 - March", "2019 - April", "2019 - May", "2019 - June"))

x_sub2_long_crop$month_label <- factor(x_sub2_long_crop$month_label)
...
ggplot() +
  ...
  scale_fill_manual(values = c(
  "1" = "slategray",  # continent
  "3" = "black",    # ice shelf / glacier
  "4" = "beige"     # land fast-ice
  ), na.value = NA) +

  new_scale_color() +
  geom_point(data = chla_at_4m, aes(x = lon, y = lat, color = chla_corrected), size = 0.3) +
  scale_color_viridis_c(limits = c(-15, 5), oob = scales::squish, na.value = NA) +

  geom_point(data = DDU_loc, aes(x = lon, y = lat), color = "red", shape = 4, size = 0.5, stroke = 0.5)+
  annotate("text", x = DDU_loc$lon, y = DDU_loc$lat, label = "DDU", hjust = -0.2, vjust = -0.5, color = "red", size = 2) +
  facet_wrap(~month_label) +
  theme_minimal() +
  labs(x = "Lon", y = "Lat", title = paste0(DEPLOY_FOLDER, "- Chla values at 4 m deep by month"))
