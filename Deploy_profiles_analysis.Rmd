...
# Convertir en salinité absolue (SA) et température conservative (CT)
SA <- gsw_SA_from_SP(sal, p = p, longitude = lon, latitude = lat)
...
contour2D(x = sa_grid, y = ct_grid, z = sigma.c, lwd = 2,
          ...
          col = "black",
          xlab = expression("Absolute Salinity (g/kg)"),
          ylab = expression("Conservative Temperature ("*~degree*C*")"))

# points TS colorés par densité
scatter2D(SA, CT, colvar = sigma_theta, pch = 16, cex = 1.25, add = TRUE,
