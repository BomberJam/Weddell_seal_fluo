...
ggplot() +
  ...
  geom_point(
    data = df_surface_clean %>%
      filter(lubridate::month(date) == month_profile),  # points du mois month_profile
    aes(x = lon, y = lat),
...
```{r}
library(sf)
profiles_per_month$mois <- lubridate::month(profiles_per_month$date)

points_sf <- st_as_sf(profiles_per_month, coords = c("lon", "lat"), crs = 4326) # Transforme en objets spatiaux
profiles_per_month$SIC <- terra::extract(amsr_spat, vect(points_sf))[,2]  # [,2] car extract retourne ID & valeur

glacier_mask <- glacier
glacier_mask[!(glacier_mask == 4)] <- 0
glacier_mask[glacier_mask == 4] <- 1

profiles_per_month$glacier_presence <- terra::extract(glacier_mask, vect(points_sf))[,2]

head(profiles_per_month)
```
