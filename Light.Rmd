...
valid_mld_indices <- which(!is.na(mld_vec))
mld_vals <- mld_vec[valid_mld_indices]

mld_moving_median <- rollapply(mld_vals, width = 180, FUN = median, na.rm = TRUE, align = "center", partial = TRUE)

plot(valid_mld_indices, mld_vals, pch = 4, col = "blue",
     ylim = rev(range(mld_vals, na.rm = TRUE)), xlim = range(0,200), xlab = "Profil", ylab = "MLD (m)")
lines(valid_mld_indices, mld_moving_median, col = "red", lwd = 2)
...
median_last10_vals <- sapply(1:ncol(light_subset), function(n) {
  col_data <- light_subset[, n]
  idx <- which(!is.na(col_data) & col_data != 0)

  if (length(idx) >= 1) {
    # Prendre les indices des 10 dernières valeurs
    last10_idx <- tail(idx, 10)
    median(col_data[last10_idx], na.rm = TRUE)
  } else {
    NA
  }
})

light_corrected <- sweep(light_subset, 2, median_last10_vals, FUN = "-")
...
abline(h = par15_depths, col = "red", lty = 2)
...
```{r}
quenchDep_temp <- apply(light, 2, function(col) {
  idx <- tail(which(col > 15), 1)
  if (length(idx) == 0) return(NA) else return(idx)
})
quenchDepth <- quenchDep_temp

# %% compute depth of quenching threshold 15 umol.m-2.s-1 (xing et al. 2018)
# 
# % lightData.quenchDepth contains depth of quenching threshold at 15 umol.m-2.s-1
# % (xing et al. 2018)
# 
# quenchDep_temp = find_ndim(L_adNonLogRegDk > 15, 1, 'last').' ;
# quenchDep_temp(quenchDep_temp == 0) = NaN ;
# 
# lightData.quenchDepth = quenchDep_temp ;
