...
vline_dates <- as.POSIXct(c("2019-02-20", "2019-03-21", "2019-04-19"))

ggplot(df_solar, aes(x = date, y = light, color = period)) +
  geom_point() +
  geom_vline(xintercept = vline_dates, linetype = "dashed", color = "black") +
  annotate("text", x = vline_dates[1], y = max(df_solar$light, na.rm = TRUE), label = "19/02\n🌙", angle = 90, vjust = -0.5, size = 3) +
...
ggplot(df_solar, aes(x = date, y = light, color = period, shape = zone)) +
  geom_point() +
  geom_vline(xintercept = vline_dates, linetype = "dashed", color = "black") +
  annotate("text", x = vline_dates[1], y = max(df_solar$light, na.rm = TRUE), label = "19/02\n🌙", angle = 90, vjust = -0.5, size = 3) +
  annotate("text", x = vline_dates[2], y = max(df_solar$light, na.rm = TRUE), label = "21/03\n🌙", angle = 90, vjust = -0.5, size = 3) +
