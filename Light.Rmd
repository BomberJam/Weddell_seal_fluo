...
## Variables
```{r variables}
MAX_CHLA_DEPTH <- 100       # define depth at which to consider dark signal (CHLA "absolute zero")
...
for (i in 1:n_profiles) {
  firstGood <- which(!is.na(temp[, i]) & !is.na(pres[, i]) & !is.na(sal[, i]))[1]
  if (!is.na(firstGood)) {
...
for (i in validProfiles) {
  ...
  if (sum(good) > 5) {
    ...
    # Trouver l'indice de référence (premier >= 10 m)
    ...
    if (!is.na(refIdx)) {
      ...
      # Trouver l'indice où le seuil est dépassé au-delà de 10 m
...
plot(valid_indices, mld_vals, pch = 4, col = "blue",
     ylim = rev(range(mld_vals, na.rm = TRUE)), xlim = range(0,150), xlab = "Profil", ylab = "MLD (m)")
...
correct_light_profile <- function(col_data, pres_vector, min_window = 4, alpha = 0.01) 
{
  # indices valides
  ...
  # balayage depuis la surface vers le fond
  ...
  if (sum(!is.na(corrected)) >= 2) {
    corrected <- na.approx(corrected, x = pres_vector, na.rm = FALSE)
  }
  return(corrected)
}

pres_vector <- pres[, 1]
light_corrected <- apply(light_subset_exp_round, 2, function(col) correct_light_profile(col, pres_vector))
light_corrected_log <- log(pmax(light_corrected, 1e-10))

```

## Profondeur de quenching basé sur le PAR 15
```{r}
# compute depth of quenching threshold 15 umol.m-2.s-1 (xing et al. 2018)
quenchDepth <- apply(light_corrected, 2, function(col) {
...
plot(light_subset_exp_round[1:MAX_CHLA_DEPTH, id_profile], pres[1:MAX_CHLA_DEPTH, id_profile], type = "l", col = "blue",
     ylim = rev(range(pres[1:MAX_CHLA_DEPTH, id_profile])), xlim = c(-0.005,5),
     xlab = "Light (µmol/m²/sec)", ylab = "Depth (m)", main = paste0("Light profile ",id_profile," - Original and Corrected\n", SEAL_NAME))

lines(light_corrected[1:MAX_CHLA_DEPTH, id_profile], pres[1:MAX_CHLA_DEPTH, id_profile], col = "red")
legend("bottomright", legend = c("Original", "Corrected"),col = c("blue", "red"), lty = 1, bty = "n")

abline(h = quenchDepth[id_profile], col = "cyan", lty = 2)
...
```{r}
matplot(light_subset_exp[1:MAX_CHLA_DEPTH,], pres[1:MAX_CHLA_DEPTH,],
        type = "l",
...
matplot(light_corrected, pres[,1],
        type = "l",
        ylim = rev(c(0, MAX_CHLA_DEPTH)),
        lty = 1,
        xlab = "Light (µmol/m²/s)",
...
nlight = ncol(light_subset_exp_round)

for (i in 1:nlight) {
  profil_light <- light_subset_exp_round[, i]
  profil_pres <- pres_vector
  ...
  abline(h = quenchDepth[i], col = "cyan", lty = 2)
...
plot(light[m,n], pres[m,n],
     type = "l",
     xlim = range(light_corrected[,n], na.rm = TRUE),
...
mtext(paste0(SEAL_NAME, " : ", n, "/", ncol(light_corrected), " (", format(date_subset[n], "%Y-%m-%d"), ")"), side = 3, line = -2, outer = TRUE)
...
extract_sic    <- terra::extract(amsr_raster_cropped, points_vect, ID = FALSE)
...
# light_subset <- light [1:MAX_CHLA_DEPTH, !col_na_only]
...
```{r DarkValue}
# 
# Calcule de la médiane de CHLA sur les 10 derniers mètres d’un profil profond (prof_depth)
# prof_depth est un offset à soustraire du profil CHLA pour chaque mesure.
# valeurs manquantes = les plus proches valides (fillmissing).
# 
# L’offset calculé est soustrait à toutes les valeurs du profil.
# Les valeurs négatives (artefacts) sont remplacées par 0.

# Détection et suppression de signaux CHLA constants en profondeur
# Pour chaque profil non vide :
# Cherche la profondeur où le signal devient constant (artefact possible).
# Si variation en profondeur < un seuil (delta_for_cstCHLA), la partie constante est mise à zéro.
# La profondeur correspondante (PDARK_temp) est enregistrée.

correct_chla_profile <- function(col_data, pres_vector, prof_depth = 10, delta_for_cstCHLA = 0.01) 
{
  ...
  # Remplissage des NA : interpolation puis voisins les plus proches
  col_data <- zoo::na.approx(col_data, x = pres_vector, na.rm = FALSE)
  col_data <- zoo::na.locf(col_data, na.rm = FALSE)
  col_data <- zoo::na.locf(col_data, fromLast = TRUE, na.rm = FALSE)

  # Profondeur max atteinte (fond du profil)
  max_depth <- max(pres_vector, na.rm = TRUE)
  bottom_limit <- max_depth - prof_depth

  # Indices correspondant aux 10 derniers mètres
  idx_bottom <- which(pres_vector >= bottom_limit)
  ...
  # Médiane des 10 derniers mètres = valeur d'offset (dark)
  dark_val <- median(col_data[idx_bottom], na.rm = TRUE)
  dark_val <- round(dark_val, 4)

  # Soustraction du dark offset
  corrected <- col_data - dark_val

  # Remplacer valeurs négatives par 0
  corrected[corrected < 0] <- 0

  # Détection de signal constant en profondeur (artefact)
  PDARK_temp <- NA
  ...
  if (!is.na(first_non_na) && !is.na(last_non_na) && (last_non_na - first_non_na + 1) >= 2) {
    for (j in first_non_na:(last_non_na - 1)) {
      ...
      if (amplitude <= delta_for_cstCHLA) {
        # On a trouvé un signal constant à partir de j+1
        ind_dark_temp <- j + 1
        corrected[ind_dark_temp:last_non_na] <- 0
        PDARK_temp <- pres_vector[ind_dark_temp]
        break
      }
    }
  }
...
pres_vector <- pres[, 1]
result_list <- lapply(1:ncol(chla), function(i) correct_chla_profile(chla[, i], pres_vector))
...
plot(chla[1:MAX_CHLA_DEPTH, nprofile], pres_vector[1:MAX_CHLA_DEPTH], type = "l", col = "blue",
     ylim = rev(range(pres_vector[1:MAX_CHLA_DEPTH], na.rm = TRUE)),
     xlim = c(0,3),
     xlab = "Chl-a", ylab = "Depth", main = paste("Profil", nprofile, "-", SEAL_NAME))

lines(chla_corrected[1:MAX_CHLA_DEPTH, nprofile], pres_vector[1:MAX_CHLA_DEPTH], col = "red")
...
```{r}
matplot(chla[1:MAX_CHLA_DEPTH,], pres_vector[1:MAX_CHLA_DEPTH],
        type = "l",
        ylim = rev(c(0, MAX_CHLA_DEPTH)),
        lty = 1,
        xlab = "Chl-a (mg/m³)",
...
matplot(chla_corrected[1:MAX_CHLA_DEPTH,], pres_vector[1:MAX_CHLA_DEPTH],
        type = "l",
        ylim = rev(c(0, MAX_CHLA_DEPTH)),
        lty = 1,
        xlab = "Light (mg/m³)",
...
#        min entre la profondeur de mélange (MLD) et la profondeur de quenching (quenchDepth)
# 
...
# flags_temp(genData.MLDphy(ii_temp) <= lightData.quenchDepth) = 2 ;
# % NIGHT PROFILE
# flags_temp(idx035_lightDay == 0) = 7 ;
# % BOTTOM OF NPQ LAYER NOT REACHED
# flags_temp(isnan(lightData.quenchDepth)) = 8 ;
# % NO LIGHT DATA
...
## Couche Eupho + PAR 15 + RSD : le cimetiere
```{r}
# COUCHE EUPHO
