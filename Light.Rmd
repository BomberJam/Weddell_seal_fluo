...
# diag$date <- as.POSIXct(diag$date, format="%Y-%m-%d %H:%M")
# 
# date_seq <- seq(from = as.POSIXct(min(dateTime), origin = "1970-01-01"),
#                 to   = as.POSIXct(max(dateTime) + 1, origin = "1970-01-01") - 3600,
#                 by = "hour")
# 
# df_wd11 <- diag %>% filter(id == SEAL_NAME)
# df_wd11 <- df_wd11 %>% arrange(date)
...
ggplot() +
  geom_point(data = df_surface_clean, 
  ...
  labs(
    title = paste0(n_profiles_surf, " light profiles (", SURFACE_NOT_NAN, "m deep) + interpolated positions"),
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal()
...
df_solar <- df_solar %>%
  mutate(period = case_when(
    solar_angle > 0 ~ "Day",
    solar_angle <= 0 & solar_angle > -12 ~ "Twilight",
    solar_angle <= -12 ~ "Night"
  ))
...
plot(new_r, legend = FALSE) # le crop

# Once per month between feb and august
