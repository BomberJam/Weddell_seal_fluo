...
last_nonzero_vals <- sapply(1:ncol(chla_subset), function(n) {
  idx <- which(!is.na(chla_subset[, n]) & chla_subset[, n] != 0)
  if (length(idx) > 0) {
    chla_subset[max(idx), n]
  }
})

# mediane des 10 dernieres profondeurs
# Remplir les NA
# Detecter signaux constants en bas de profil
# Verifier que le profil est bien profond

nprofile=62

chla_corrected <- sweep(chla_subset, 2, last_nonzero_vals, FUN = "-") # dark-value ? Not sure mhmhmhm

# Plot only one of the 150
plot(chla_subset[1:MAX_CHLA_DEPTH, nprofile], pres[1:MAX_CHLA_DEPTH, nprofile], type = "l", col = "blue",
     ylim = rev(range(pres[1:200, nprofile], na.rm = TRUE)),
     xlim = c(-5,20),
     xlab = "Chl-a", ylab = "Depth", main = paste("Profil", nprofile, "- Dark value: ", last_nonzero_vals[nprofile]))

lines(chla_corrected[1:MAX_CHLA_DEPTH, nprofile], pres[1:MAX_CHLA_DEPTH, nprofile], col = "red")
...
```{r}
matplot(chla[1:MAX_CHLA_DEPTH,], pres[1:MAX_CHLA_DEPTH,],
        type = "l",
        ylim = rev(c(0, MAX_CHLA_DEPTH)),
        lty = 1,
        xlab = "Chl-a (mg/m³)",
...
matplot(chla_corrected, pres[1:MAX_CHLA_DEPTH,],
        type = "l",
        ylim = rev(c(0, MAX_CHLA_DEPTH)),
        lty = 1,
        xlab = "Light (mg/m³)",
