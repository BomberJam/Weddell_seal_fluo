...
combined_data <- files %>%
  lapply(readRDS) %>%
  bind_rows() %>%
  arrange(seal_id, profile_id, date) # trier par ind, puis profile, puis date.
...
# Suppression profils Temp trop haute
```{r}
library(dplyr)

df <- readRDS("/home/cactus/Documents/Oceano/M2/LOCEAN/Weddell_seal_fluo/oceanographic_data_corrected/all_deployment.rds")

seuil <- 4  # remplace par la valeur souhaitée
valeurs_sup <- unique(df$temperature[df$temperature > seuil])

df_filtered <- df[df$temperature <= seuil, ]

nb_suppr <- nrow(df) - nrow(df_filtered)
